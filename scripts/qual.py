#!/usr/bin/env python3
"""
Script to plot qualitative results for a supervised model.
...
def main(
    ...
    **kwargs,
    ...
    _, dl, _, patch_h, _, n_classes = get_dataloaders(
        dataset, seq_len, patch_len, batch_size, split, False, False, logger, seed
    )

    preds = []
    for i, model in enumerate(["u", "nlur", "aspp"]):
        # Model
        model_name = model
        in_channels = 1
    ...
    plt.savefig(out_dir + "/qual_supervised.pdf", dpi=300)
    plt.close()
